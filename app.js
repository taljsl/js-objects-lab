...
game.party.sort((least, most) => most.hp - least.hp);
...
game.collection = [];
// game.party.push(pokemonObj)(original function after arrow)
game.catchPokemon = (pokemonObj) => {
...
game.catchPokemon = (pokeName) => {
  //This function built w/ cousin who is programmer, he reminded me to initialize variable for readability
  const pokemonNameLowered = pokeName.toLowerCase();
  for (let i = 0; i < pokemon.length; i++) {
    const currentPokemon = pokemon[i]; //current mon ivysaur
    if (pokemonNameLowered === currentPokemon.name.toLowerCase()) {
      //bulb != ivy
      if (game.items[1].quantity === 0) {
        //game.items[1] is pokeballs
        console.log("You are out of pokeballs");
        ...
        game.party.push(currentPokemon);
      } else {
        game.collection.push(currentPokemon);
      }
      game.items[1].quantity = game.items[1].quantity - 1;
      return;
    }
  }
  console.log(`That pokemon does not exist, please try again`);
};
...
/*
Write a function that accepts pokemon data and returns an object containing several arrays of pokemon types
*/